...
InventoryClass.InventoryClassName_lev1 as invClassName   from ST_RDDetail as RDRecord
 ...
 where   {RDRecord.IDwarehouse_$warehouseDTO} and {RDRecord.IDInventory_$inventoryDTO}  and 
  {rdrecordvoucherdate} 
 group by InventoryClass.InventoryClassName_lev1 having abs(SUM(isnull(RDRecord.ArrivalAmount,0)-isnull(RDRecord.DispatchAmount,0)))>0.005 or SUM(isnull(RDRecord.ArrivalBaseQuantity,0)-isnull(RDRecord.DispatchBaseQuantity,0))<>0'
