...
(function($ , undefined) {
	...
	var Ace_File_Input = function(element , settings) {
		...
		this.$element.wrap('<div class="tp-file-input" />');
	...
	Ace_File_Input.prototype.apply_settings = function() {
		...
		if(this.well_style) this.$element.parent().addClass('tp-file-multiple');
		 else this.$element.parent().removeClass('tp-file-multiple');
		...
		if(remove_btn) this.$label.next('a').on(tp.click_event, function(){
	...
	$.fn.ace_spinner = function(options) {
		...
		this.each(function() {
			...
			max = (''+max).length
				...
				$(this).addClass('spinner-input form-control').wrap('<div class="tp-spinner">')
				var $parent_div = $(this).closest('.tp-spinner').spinner(options).wrapInner("<div class='input-group'></div>")
...
(function($ , undefined) {
	$.fn.ace_wizard = function(options) {
		...
		this.each(function() {
			...
			$wizard.$prevBtn = buttons.find('.btn-prev').eq(0).on(tp.click_event,  function(){
				$this.wizard('previous');
			}).attr('disabled', 'disabled');
			$wizard.$nextBtn = buttons.find('.btn-next').eq(0).on(tp.click_event,  function(){
				$this.wizard('next');
			}).removeAttr('disabled');
...
(function($ , undefined) {
	$.fn.ace_colorpicker = function(options) {
		...
		this.each(function() {
			...
			.next().find('.dropdown-menu').on(tp.click_event, function(e) {
				var a = $(e.target);
...
(function($ , undefined) {
	$.fn.ace_wysiwyg = function($options , undefined) {
		...
		this.each(function() {
			...
			toolbar.find('.dropdown-menu input:not([type=file])').on(tp.click_event, function() {return false})
			...
			toolbar.find('input[type=file]').prev().on(tp.click_event, function (e) { 
				$(this).next().click();
			});
