...
jQuery(function ($) {
    ...
    window['tp'].click_event = $.fn.tap ? "tap" : "click";
});

jQuery(function ($) {
    //tp.click_event defined in tp-elements.js
    tp.handle_side_menu(jQuery);

    tp.enable_search_ahead(jQuery);

    tp.general_things(jQuery);//and settings

    tp.widget_boxes(jQuery);
    tp.widget_reload_handler(jQuery);//this is for demo only, you can remove and have your own function, please see examples/widget.html
...
tp.handle_side_menu = function ($) {
    $('#menu-toggler').on(tp.click_event, function () {
    ...
    $('#sidebar-collapse').on(tp.click_event, function () {
        $minimized = $('#sidebar').hasClass('menu-min');
        tp.settings.sidebar_collapsed(!$minimized);//@ tp-extra.js
    });

    var touch = "ontouchend" in document;

    var curmenu = $('.nav-list').find('> .open > .submenu > .active').get(0);
    //opening submenu
    $('.nav-list').on(tp.click_event, function (e) { 
        ...
        if (!link_element.hasClass('dropdown-toggle')) {//it doesn't have a submenu return
            ...
            if ($minimized && tp.click_event == "tap" &&
            ...
            if (curmenu) {
                ...
                //$(curmenu).parent().parent().removeClass('active');
            }
            curmenu = link_element.parent();
            window.localStorage ? window.localStorage.setItem('curview', link_element.attr('href')) : '';
            $(curmenu).addClass('active');
            //$(curmenu).parent().parent().addClass('active');
    ...
    //localStorage curview
    var curview = window.localStorage ? window.localStorage.getItem('curview') : '';
    if (curview) {
        $('.nav-list').find('[href=' + curview.replace(/#/g, '\\#').replace(/\//g, '\\/') + ']').each(function () {
            $(this).click();
            this.click();
        });
    }
...
tp.general_things = function ($) {
    $('.tp-nav [class*="icon-animated-"]').closest('a').on('click', function () {
    ...
    $('#tp-settings-btn').on(tp.click_event, function () {
        $(this).toggleClass('open');
        $('#tp-settings-box').toggleClass('open');
    });


    $('#tp-settings-navbar').on('click', function () {
        tp.settings.navbar_fixed(this.checked);//@ tp-extra.js
    }).each(function () { this.checked = tp.settings.is('navbar', 'fixed') })

    $('#tp-settings-sidebar').on('click', function () {
        tp.settings.sidebar_fixed(this.checked);//@ tp-extra.js
    }).each(function () { this.checked = tp.settings.is('sidebar', 'fixed') })

    $('#tp-settings-breadcrumbs').on('click', function () {
        tp.settings.breadcrumbs_fixed(this.checked);//@ tp-extra.js
    }).each(function () { this.checked = tp.settings.is('breadcrumbs', 'fixed') })

    $('#tp-settings-add-container').on('click', function () {
        tp.settings.main_container_fixed(this.checked);//@ tp-extra.js
    }).each(function () { this.checked = tp.settings.is('main-container', 'fixed') })

    //Switching to RTL (right to left) Mode
    $('#tp-settings-rtl').removeAttr('checked').on('click', function () {
        tp.switch_direction(jQuery);
    });


    $('#btn-scroll-up').on(tp.click_event, function () {
    ...
    $('#skin-colorpicker').on('change', function () {
        ...
        if (skin_class == 'skin-1') {
            $('.tp-nav > li.grey').addClass('dark');
        }
        else {
            $('.tp-nav > li.grey').removeClass('dark');
        }

        if (skin_class == 'skin-2') {
            $('.tp-nav > li').addClass('no-border margin-1');
            $('.tp-nav > li:not(:last-child)').addClass('light-pink').find('> a > [class*="icon-"]').addClass('pink').end().eq(0).find('.badge').addClass('badge-warning');
        }
        else {
            $('.tp-nav > li').removeClass('no-border margin-1');
            $('.tp-nav > li:not(:last-child)').removeClass('light-pink').find('> a > [class*="icon-"]').removeClass('pink').end().eq(0).find('.badge').removeClass('badge-warning');
        }

        if (skin_class == 'skin-3') {
            $('.tp-nav > li.grey').addClass('red').find('.badge').addClass('badge-yellow');
        } else {
            $('.tp-nav > li.grey').removeClass('red').find('.badge').removeClass('badge-yellow');
        }
    });
...
tp.widget_boxes = function ($) {
    ...
    $(document).on('click.tp.widget', '[data-action]', function (ev) {
        ev.preventDefault();
        ...
        if ($action == 'collapse') {
            ...
            var event
            $box.trigger(event = $.Event(event_name + '.tp.widget'))
            ...
            if (event_name == 'show') {
                ...
                $body.slideUp(0, function () { $body.slideDown(expandSpeed, function () { $box.trigger(event = $.Event(event_complete_name + '.tp.widget')) }) });
            }
            else {
                if ($icon) $icon.addClass($icon_down).removeClass($icon_up);
                $body.slideUp(collapseSpeed, function () { $box.addClass('collapsed'); $box.trigger(event = $.Event(event_complete_name + '.tp.widget')) });
            }
            ...
            var event
            $box.trigger(event = $.Event('close.tp.widget'))
            if (event.isDefaultPrevented()) return

            var closeSpeed = parseInt($this.data('close-speed')) || 300;
            $box.hide(closeSpeed, function () { $box.trigger(event = $.Event('closed.tp.widget')); $box.remove(); });
        }
        else if ($action == 'reload') {
            var event
            $box.trigger(event = $.Event('reload.tp.widget'))
            if (event.isDefaultPrevented()) return
            ...
            $box.one('reloaded.tp.widget', function () {
            ...
            var event = $.Event('settings.tp.widget')
            $box.trigger(event)
        }
...
tp.widget_reload_handler = function ($) {
    //***default action for reload in this demo
    //you should remove this and add your own handler for each specific .widget-box
    //when data is finished loading or processing is done you can call $box.trigger('reloaded.tp.widget')
    $(document).on('reload.tp.widget', '.widget-box', function (ev) {
        var $box = $(this);
        //trigger the reloaded event after 1-2 seconds
        setTimeout(function () {
            $box.trigger('reloaded.tp.widget');
    ...
    /**
	$('#my-widget-box').on('reload.tp.widget', function(){
		//load new data 
		//when finished trigger "reloaded"
		$(this).trigger('reloaded.tp.widget');
	});	
	*/
}
...
tp.enable_search_ahead = function ($) {
    tp.variable_US_STATES = ["Alabama", "Alaska", "Arizona", "Arkansas", "California", "Colorado", "Connecticut", "Delaware", "Florida", "Georgia", "Hawaii", "Idaho", "Illinois", "Indiana", "Iowa", "Kansas", "Kentucky", "Louisiana", "Maine", "Maryland", "Massachusetts", "Michigan", "Minnesota", "Mississippi", "Missouri", "Montana", "Nebraska", "Nevada", "New Hampshire", "New Jersey", "New Mexico", "New York", "North Dakota", "North Carolina", "Ohio", "Oklahoma", "Oregon", "Pennsylvania", "Rhode Island", "South Carolina", "South Dakota", "Tennessee", "Texas", "Utah", "Vermont", "Virginia", "Washington", "West Virginia", "Wisconsin", "Wyoming"]

    try {
        $('#nav-search-input').typeahead({
            source: tp.variable_US_STATES,
